...
function getLatLongOrigen() {
	...
	var encontre = false;
	for (i = 0;
		(i < vectorDePuntosJSON.length && !encontre); i++) {
...
function doDescartoSiNoEstaLaEntrega(PedidoId) {
	var encontre = false;
	for (x = 0;
		(x < vectorDePuntosJSON.length) && (!encontre); x++) {
...
function rutear(rutear) {
	...
	if (vectorDePuntosJSON.length == 0) {
		return
	};
		...
		for (x = 0; x < tope; x++) {
			if (!esModRuta()) {
				var latlng = new google.maps.LatLng({
					lat: vectorDePuntosJSON[x].PuntoLat,
					lng: vectorDePuntosJSON[x].PuntoLong
				});
...
function agregaPuntoEnElMapa(Punto) {
	...
	marker = new google.maps.Marker({
		...
		icon: {
			url: icono,
			scaledSize: new google.maps.Size(hSIZE_SVG, vSIZE_SVG)
		},
	...
	google.maps.event.addListener(marker, 'dragend', function(e) {
		...
		if (hayRuta){
			...
			mostrarRegistrosRuta(getZonaEnUso());
		}
	});
...
function crearOrdenVista(ZonaId){
	orden =0;
	for (var i = 0; i < vectorDePuntosJSON.length; i++) {
		...
		if (vectorDePuntosJSON[i].PuntoZonaId == ZonaId) {
			...
			if (!vectorDePuntosJSON[i].PuntoOcultar){
				if ((vectorDePuntosJSON[i].Precedencia == 2) || (vectorDePuntosJSON[i].Precedencia == 0) || (vectorDePuntosJSON[i].PedidosMuestraAmbos)){
					...
					vectorDePuntosJSON[i].PuntoOrdenVista = -1;
				}
			}else{
				vectorDePuntosJSON[i].PuntoOrdenVista = vectorDePuntosJSON[i].PuntoOrden;
...
function getIcono(opcion) {
	...
	switch (opcion) {
		...
		default:
			...
			msg('!!!!No se encontro el icono ' + opcion);
			break;
	}
	return icono;
}
...
function modificarPuntoEnElMapa(Punto) {
	...
	for (i = 0;
		((i < vecMarkersPuntos.length) && (!encontre)); i++) {
		if (vecMarkersPuntos[i].labelContent == Punto.PuntoId) {
			vecMarkersPuntos[i].position = new google.maps.LatLng({
				lat: Punto.PuntoLat,
				lng: Punto.PuntoLong
			});
...
function getLatLongWareHouseNoGoogle() {
	...
	for (var i = 0;
		((i < vectorOrigenes.length) && (!encontre)); i++) {
		if (vectorOrigenes[i].OrigenDefault == "S") {
			o = vectorOrigenes[i];
			latlng = {
				lat: Number(o.OrigenLat),
				lng: Number(o.OrigenLong)
			};
			encontre = true;
		}
	}
...
function SuperaElTopeDePuntosParaElServDeRuta(ZonaId) {
	...
	var cont = 0;
	for (i = 0;
		(i < vectorDePuntosJSON.length && !supera); i++) {
		if (vectorDePuntosJSON[i].PuntoZonaId == ZonaId) {
			cont++;
		...
		if ((vectorDePuntosJSON[i].PuntoZonaId == getZonaEnUso()) && (vectorDePuntosJSON[i].FlagRuteo == true)) {
			...
			if (ultimalatlng == null) {
				...
				if (vectorDePuntosJSON[i].Marca == vMarcaDESTINO || c == 1) {
					ultimalatlngGeneral = new google.maps.LatLng({
						lat: vectorDePuntosJSON[i].PuntoLat,
						lng: vectorDePuntosJSON[i].PuntoLong
					});
				}
			}
			...
			if (cant == TOPE_PUNTOS_SERV_RUTA) {
				...
				ultimalatlng = new google.maps.LatLng({
					lat: vectorDePuntosJSON[i].PuntoLat,
					lng: vectorDePuntosJSON[i].PuntoLong
				});
				if (waypts.length == 0) {
				...
				var directionsDisplay = new google.maps.DirectionsRenderer({
					suppressMarkers: refletras,
					polylineOptions: {
						strokeColor: vecColor[((tandas + 10) % 5)]
					}
				});
...
function OrdenarPorHora() {
	...
	vectorDePuntosJSON.sort(function(a, b) {
		...
		var distancia = calcdistancia(new google.maps.LatLng({
			lat: a.PuntoLat,
			lng: a.PuntoLong
...
function movimientoValido(PuntoAMover, PuntoOrdenQueOcuparia) {
	...
	for (var ii = 0; ii < vectorDePuntosJSON.length; ii++) {
		if (vectorDePuntosJSON[ii].PuntoZonaId == PuntoAMover.PuntoZonaId) {
			...
			if (!vectorDePuntosJSON[ii].PuntoHabilitado) {
				/*orden que ocuparia debe ser mayor que el ultimo trancado */
			...
			if (vectorDePuntosJSON[i].PuntoId == PuntoId) {
				if (direccion == "D") { //Si se mueve hacia abajo (incrementa la posición)
					if (i < (vectorDePuntosJSON.length - 1)) {
						...
						for (var j = posinicial;
							((!vectorDePuntosJSON[j].FlagRuteo) || (vectorDePuntosJSON[j].PuntoZonaId != getZonaEnUso()) && (j < vectorDePuntosJSON.length)); j++) {
					...
					if (i > 0) {
						...
						for (var j = posinicial;
							((!vectorDePuntosJSON[j].FlagRuteo) || (vectorDePuntosJSON[j].PuntoZonaId != getZonaEnUso()) && (j > 0)); j--) {
...
function getPolyById(id) {
	...
	var poly = undefined;
	for (i = 0;
		((i < vecPoligonos.length) && (!encontre)); i++) {
...
function limpiarRuta() {
	for (var i = 0;
		(i < vecDireccionDisplay.length); i++) {
...
function extistePolyAlArray(tag) {
	...
	var existe = false;
	for (i = 0;
		((i < vecPoligonos.length) && (!existe)); i++) {
...
function limpiarPoligono(poly) {
	...
	for (i = 0;
		((i < vecPoligonos.length) && (!encontro)); i++) {
...
function armarListaOrdenada(posPrimerElemento, listaDesordenada) {
	...
	var v = {
		PuntoId: ele.PuntoId,
		PedidoId: ele.PedidoId,
		Precedencia: ele.Precedencia,
		Posicion: posele
	};
	...
	for (i = 0; i < lista.length; i++) {
		...
		if ((ele.PuntoId != p.PuntoId) && (p.PuntoZonaId == getZonaEnUso())) {
			var distancia = calcdistancia(new google.maps.LatLng({
				...
				lat: p.PuntoLat,
				lng: p.PuntoLong
			})); //distanciaEntre2Puntos(ele.PuntoLat, ele.PuntoLong, p.PuntoLat, p.PuntoLong);
			if (hordenarPorHora) {
...
function cargarJsonPunto(PedidoId) {
	...
	$.ajax({
		...
		error: function() {
			alert('Falló la carga de Puntos');
		}
	});
}
